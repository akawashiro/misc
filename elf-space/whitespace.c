...
/* Notes
        ...
        I am aware that I could have incremented instruction_index in the step
   through function, however it makes more sense to me to do it in the actual
   instruction. MAX_LABEL_LENGTH also applies to maximum length for a number (in
   bits or whitespace characters) The plan was to use dynamic memory allocation
   throughout the entire project, but I ran into a couple sections where I kept
   getting mem bugs that I couldn't fix, so I had to switch to static allocation
   in those areas
*/
...
int create_instruction_set(void) {
  ...
  instruction_set.instruction_function =
      calloc(MAX_INSTRUCTIONS, sizeof(void (*)(char *, int)));
...
int locate_jump_labels(char *source) {
  ...
  for (int i = 0; source[i]; i++) {
    if (!strncmp(&(source[i]), label_marker, 3) && (source[i - 1] != '\x09')) {
      if (label_table.label_id[label_index] =
              (char *)calloc(MAX_LABEL_LENGTH, sizeof(char))) {
        ...
        for (j = 0; source[i + j] != '\x0A'; j++)
          label_table.label_id[label_index][j] = source[i + j];
        i += j;
        label_table.label_location[label_index] =
            i + 1; // The 1 puts it past the final \x0A
        label_index++;
        continue;
      }
...
void step_through_program(char *source) {
  ...
  while (current_instruction_index != -1 && source[current_instruction_index]) {
    for (i = 0; i < MAX_INSTRUCTIONS; i++) {
      if (!strncmp(&(source[current_instruction_index]),
                   instruction_set.unique_id[i],
                   instruction_set.instruction_size[i])) {
        instruction_set.instruction_function[i](
            &(source[current_instruction_index]),
            instruction_set.instruction_size[i]);
        break;
      }
    }
...
void ws_stack_slide(char *parameter, int size) {
  ...
  char *label_number = NULL;
  if (leap = retrieve_label_or_number(&(parameter[size]), &label_number)) {
    for (int slide_amt = convert_ws_to_number(label_number); slide_amt > 0;
         slide_amt--)
      stack_pop();
    stack_push(first);
    current_instruction_index += size + leap + 1;
