...
cmake -G Ninja -DCMAKE_BUILD_TYPE=RelWithDebInfo -DLLVM_ENABLE_ASSERTIONS=ON -S ${LLVM_SRC_DIR}/llvm -B ${LLVM_BUILD_DIR} -DLLVM_ENABLE_PROJECTS="mlir;llvm" -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_C_COMPILER=$(which clang-14) -DCMAKE_CXX_COMPILER=$(which clang++-14) -DLLVM_ENABLE_LIBEDIT=0
